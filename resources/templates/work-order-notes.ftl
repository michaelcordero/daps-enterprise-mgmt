<#-- @ftlvariable name="presenter" type="presenters.WebWorkOrderNotesPresenter" -->
<#import "dashboard-ui.ftl" as ui />
<@ui.dashboardUI title="Work Order Notes">
    ...
    <main class="content">
        ...
        <div class="row">
            <div class="col">
                <div class="card">
                    <div class="card-body">
                        <table id="data-work-order-notes"
    ...
    <script>
        $(function () {
            ...
            web_socket.onmessage = function(event) {
                ...
                if(event_data['UPDATE'] === 'WORK_ORDER_NOTES') {
                    table.ajax.reload(null, false)
                } else if (event_data['ALERT'] === 'WORK_ORDER_NOTES') {
            ...
            const editor = new $.fn.dataTable.Editor({
                ...
                table: '#data-work-order-notes',
                idSrc: 'id',
                fields: [
                    {label: 'ID', name: 'id'},
                    {label: 'Work Order#', name: 'wo_number',attr: {required: true}},
                    {label: 'Note Date', name: 'note_date'},
                    {label: 'Initial', name: 'initial'},
                    {label: 'Comments', name: 'comments'},
                    {label: 'Follow Up Date', name: 'followup_date'},
                ]
            });
            editor.disable('id')
            // Datatables basic
            const table = $('#data-work-order-notes').DataTable({
                "ajax": {
                    "url": '/web/work_order_notes',
                    "dataSrc": ''
                },
